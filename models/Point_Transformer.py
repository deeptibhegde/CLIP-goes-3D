...
class PointTransformerProject(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        # self.decoder = PointConvDensityClsSsg_Decoder()
...
class MaskTransformer(nn.Module):
    ...
    def forward(self, neighborhood, center, return_all_tokens = False, only_cls_tokens = False, noaug = False):
        ...
        # out_feature  = torch.cat([x[:, 1:].max(1)[0]], dim = -1)

        # only return the cls feature, for moco contrast
        if only_cls_tokens:
            return out_feature,self.cls_head(x[:, 0])
        logits = self.lm_head(x[:, 1:])

        if return_all_tokens:
            return out_feature,self.cls_head(x[:, 0]), logits
        else:
            # return the flake logits
            return out_feature,self.cls_head(x[:, 0]), logits[~overall_mask], logits[overall_mask], overall_mask # reduce the Batch dim        
...
class Point_BERT(nn.Module):
    ...
    def forward_eval(self, pts):
        with torch.no_grad():
            neighborhood, center = self.group_divider(pts)
            out_feature,cls_feature = self.transformer_q(neighborhood, center, only_cls_tokens = True, noaug = True)
            return out_feature,cls_feature
    ...
    def forward(self, pts, noaug = False, **kwargs):
        if noaug:
            ...
            if self.return_all_tokens:
                out_feature ,q_cls_feature, logits = self.transformer_q(neighborhood, center, return_all_tokens = self.return_all_tokens) # logits :  N G C 
            else:
                out_feature , q_cls_feature, real_logits, flake_logits, mask = self.transformer_q(neighborhood, center, return_all_tokens = self.return_all_tokens) # logits :  N' C where N' is the mask.sum() 
            ...
            if self.return_all_tokens:
                _,mixup_cls_feature, mixup_logits = self.transformer_q(mixup_neighborhood, mixup_center, return_all_tokens = self.return_all_tokens) 
            else:
                _,mixup_cls_feature, mixup_real_logits, mixup_flake_logits, mixup_mask = self.transformer_q(mixup_neighborhood, mixup_center, return_all_tokens = self.return_all_tokens)
            mixup_cls_feature = nn.functional.normalize(mixup_cls_feature, dim=1)

            # compute key features
            with torch.no_grad():  # no gradient to keys
                self._momentum_update_key_encoder()  # update the key encoder
                _, k_cls_feature = self.transformer_k(neighborhood, center, only_cls_tokens = True)  # keys: NxC
            ...
            return out_feature,moco_loss + dvae_loss, cutmix_loss



@MODELS.register_module()
class Point_BERTProject(nn.Module):
    def __init__(self, config):
        super().__init__()
        print_log(f'[Point_BERT] build dVAE_BERT ...', logger ='Point_BERT')
        self.config = config
        ...
        self.dvae = DiscreteVAE(config.dvae_config)
        self.projection = torch.nn.Conv1d(in_channels=768,out_channels=512,kernel_size=1)
        self._prepare_dvae()
    ...
    def forward_eval(self, pts):
        with torch.no_grad():
            ...
            out_feature,cls_feature = self.transformer_q(neighborhood, center, only_cls_tokens = True, noaug = True)
            # import pdb; pdb.set_trace()
            out_feature = self.projection(out_feature.unsqueeze(-1))
            return out_feature.squeeze(), cls_feature
