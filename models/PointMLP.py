...
class ModelFuse(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="center",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[2, 2, 2, 2], **kwargs):
        super(ModelFuse, self).__init__()
        self.stages = len(pre_blocks)
        self.class_num = class_num
        self.points = points
        self.embedding = ConvBNReLU1D(3, embed_dim, bias=bias, activation=activation)
        ...
        self.act = get_activation(activation)
        self.down_proj = nn.Linear(last_channel+512,last_channel)
        self.classifier = nn.Sequential(
            nn.Linear(last_channel, 512),
            nn.BatchNorm1d(512),
            self.act,
    ...
    def forward(self, x,latent_img):
        ...
        x = F.adaptive_max_pool1d(x, 1).squeeze(dim=-1)
        # import pdb; pdb.set_trace()
        concat_feat = torch.cat((x,latent_img),dim=-1)
        concat_feat = self.down_proj(concat_feat)
...
def pointMLPFuse(num_classes=40, **kwargs) -> ModelFuse:
    return ModelFuse(points=1024, class_num=num_classes, embed_dim=64, groups=1, res_expansion=1.0,
                   activation="relu", bias=False, use_xyz=False, normalize="anchor",
                   dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                   k_neighbors=[24, 24, 24, 24], reducers=[2, 2, 2, 2], **kwargs)
