...
from models.PointMLP import pointMLPFuse
from models.PointConv import PointConvDensityClsSsg as PointConv
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if config.model.NAME == 'PointMLPFuse':
        base_model = pointMLPFuse()
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            # import pdb; pdb.set_trace()
            if config.model.NAME == 'PointMLPFuse' or config.model.NAME == 'PointConv':
                ret = base_model(points.permute(0,2,1).contiguous(),latent_img)
            else:
                ret = base_model(points,latent_img)
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            metrics = validate(base_model,clip_model, test_dataloader, epoch, val_writer, args, config, logger=logger)
            ...
            if metrics.acc > 91.5 or (better and metrics.acc > 90):
                metrics_vote = validate_vote(base_model,clip_model, test_dataloader, epoch, val_writer, args, config, logger=logger)
...
def validate(base_model,clip_model, test_dataloader, epoch, val_writer, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if config.model.NAME == 'PointMLPFuse' or config.model.NAME == 'PointConv':
                logits = base_model(points.permute(0,2,1).contiguous(),latent_img)
            else:
                logits = base_model(points,latent_img)
            target = label.view(-1)
...
def validate_vote(base_model,clip_model, test_dataloader, epoch, val_writer, args, config, logger = None, times = 10):
